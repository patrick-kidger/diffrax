...
def _maybe_set(pred, xs, x, i, *, kwargs, makes_false_steps):
    ...
    x = u.math.asarray(x).astype(dtype)
    x = u.math.broadcast_to(x, jax.eval_shape(lambda: xs[i]).shape)
    i_dynamic, i_static = partition(i, is_array)
    i_dynamic_leaves, i_treedef = jtu.tree_flatten(i_dynamic)
    unit = u.get_unit(x)
    assert unit == u.get_unit(x), (
        "Units must match when setting a buffer. "
        f"Got {u.get_unit(xs)} and {u.get_unit(x)}."
    )
    ...
    return u.maybe_decimal(u.Quantity(out, unit=unit))
