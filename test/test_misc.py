...
def test_ω_add_mul(getkey):
    ...
    for treedef in treedefs:
        ...
        e2 = jax.tree_util.tree_map(lambda ai, bi, ci: ai * 2 + bi * ci - 3, a, b, c)
...
def test_ω_inplace(getkey):
    ...
    for treedef in treedefs:
        ...
        b1 = ω(a).at[()].set(3).ω
        b2 = jax.tree_util.tree_map(lambda ai: ai.at[()].set(3), a)
        assert shaped_allclose(b1, b2)

        a2 = jax.tree_util.tree_map(lambda x: x + 1, a)

        b3 = ω(a).at[()].set(ω(a2)).ω
        b4 = jax.tree_util.tree_map(lambda ai, a2i: ai.at[()].set(a2i[()]), a, a2)
