...
def test_rattle():
        ...
        rat = diffrax.Rattle(nonlinear_solver=diffrax.NewtonNonlinearSolver(rtol=1e-4, atol=1e-6),constrain=constrain)
        ...
        terms = (diffrax.ODETerm(lambda t, q, args: -jax.grad(H, argnums=1)(jnp.zeros_like(q), q)),
                 diffrax.ODETerm(lambda t, p, args: jax.grad(H, argnums=0)(p, jnp.zeros_like(p)))
                 )
        #p,q
        ...
        p1,q1 = solution.ys
        assert  np.allclose(p1,jnp.asarray([0.,-1.]), rtol=1e-4, atol=1e-4) 
        assert  np.allclose(q1, jnp.asarray([1., 0.]), rtol=1e-4, atol=1e-4) 
