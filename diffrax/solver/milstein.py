...
class ItoMilstein(AbstractItoSolver):
    ...
    def step(
        ...
        #
        leaves_Δw, tree_Δw = jax.tree_util.tree_flatten(Δw)
        leaves_ΔwΔw = []
        ...
        y_treedef = jax.tree_util.tree_structure(y0)
        Δw_treedef = jax.tree_util.tree_structure(Δw)
        ...
        v0_matrix = jax.tree_util.tree_map(_to_treemap, Δw, g0_matrix)
        ...
        def _dot(_, _v0):
            ...
            # ΔwΔw has structure (tree(Δw), tree(Δw), leaf(Δw), leaf(Δw))
            _dotted = jax.tree_util.tree_map(__dot, _v0, ΔwΔw)
        ...
        # v0_matrix has structure (tree(y0), tree(Δw), tree(Δw), leaf(y0), leaf(Δw), leaf(Δw))  # noqa: E501
        v0_prod = jax.tree_util.tree_map(_dot, y0, v0_matrix)
